...
async function displayBasicStatus(selectedCharacter) {
    console.log("基本ステータスと耐性、技能の設定 : ", selectedCharacter)
    baseValues = {};
    increaseValues = {};
    ...
    console.log("ステータスの設定 : ", baseValues, increaseValues, calculateCorrection(parseInt(baseValues.SIZ + increaseValues.SIZ) ))
    ...
    console.log("DEBUG: baseBodys['SIZ']:", baseBodys['SIZ'], "-> 使用値:", baseSIZ);
    console.log("DEBUG: increaseBodys['SIZ']:", increaseBodys['SIZ'], "-> 使用値:", increaseSIZ);
    console.log("DEBUG: baseBodys['足']:", baseBodys['足'], "-> 使用値:", base足);
    console.log("DEBUG: increaseBodys['足']:", increaseBodys['足'], "-> 使用値:", increase足);
    ...
    // デバッグログ: 合計値を確認
    console.log("DEBUG: SIZ合計 (baseSIZ + increaseSIZ):", SIZSum);
    console.log("DEBUG: 足合計 (base足 + increase足):", 足Sum);

    // 基本移動速度を計算 基礎値にするなら 3.5
    const movementSpeedBase = SIZSum / 30 + 10 + (7.5 + 足Sum) + (baseValues.速度 / 5 * 1) ;

    // デバッグログ: 基本移動速度を確認
    console.log("DEBUG: 基本移動速度 (SIZSum / 30 + 10 + (7.5 + 足Sum)):", movementSpeedBase);
    ...
    // デバッグログ: 各ボーナスを確認
    console.log("DEBUG: itemBonuses['移動']:", selectedCharacter.itemBonuses.stats['移動'], "-> 使用値:", itemBonus移動);
    console.log("DEBUG: skillBonuses['移動']:", selectedCharacter.skillBonuses['移動'], "-> 使用値:", skillBonus移動);

    // 増加ボーナス合計を計算
    const movementSpeedIncrease = itemBonus移動 + skillBonus移動;

    // デバッグログ: 増加ボーナスを確認
    console.log("DEBUG: 増加ボーナス (itemBonus移動 + skillBonus移動):", movementSpeedIncrease);
    ...
    // デバッグログ: 移動速度設定の最終結果
    console.log("移動速度の設定完了:");
    console.log("  基本移動速度 (baseBodys['移動']):", baseBodys['移動']);
    console.log("  増加ボーナス (increaseBodys['移動']):", increaseBodys['移動']);
    // +(速度/5)*3.5)

    console.log("基礎値と上昇値設定:", baseValues, increaseValues, baseResistances, increaseResistances, baseSkills, increaseSkills, baseBodys, increaseBodys);
    ...
    if (sizBonus > 0) {
        ...
        console.log("攻撃ボーナス:", attackBonus);
        console.log("速度ペナルティ:", speedPenalty);
        console.log("威圧ボーナス:", intimidationBonus);
        console.log("隠密ペナルティ:", stealthPenalty);
        console.log("外皮ボーナス:", armorBonus);
        ...
        // 結果をログに出力
        console.log("更新後の増加値:");
        console.log("HP:", increaseValues.HP);
        console.log("攻撃:", increaseValues.攻撃);
        console.log("速度:", increaseValues.速度);
        console.log("威圧:", increaseSkills.威圧);
        console.log("隠密:", increaseSkills.隠密);
        console.log("外皮:", increaseBodys.外皮);
    }
    ...
    console.log("肉体部位の威力設定　:", calculatedPowers);
...
function updateAttackOption(value, newData) {
    ...
    console.log("updateAttackOption 攻撃手段を更新する :", value, newData )

    const option = attackOptions.find(option => option.value === value);

    console.log("updateAttackOption option :", option )
    ...
    if (option) {
        ...
        console.log(`Error: ${value} のデータが見つかりません`);
    }
}
...
function toggleTotalView() {
    ...
    console.log("合計値表示の切り替え関数 : ", bodyType)
...
const ITEM_DETAIL_MODAL_ID = "item-detail-modal";
const DETAIL_MODAL_TYPES = {
    ITEM: "item",
    SKILL: "skill"
};
let itemDetailKeydownBound = false;
...
function getItemTypeLabel(item) {
    const rawType = item?.タイプ ?? item?.種類 ?? item?.type;
    const normalizedType = String(rawType ?? "").trim();
    return normalizedType || "タイプ情報なし";
}

function getItemDescriptionText(item) {
    const normalizedDescription = String(item?.説明 ?? "").trim();
    return normalizedDescription || "詳細情報なし";
}
...
function getSkillDescriptionText(skill) {
    const normalizedDescription = String(skill?.説明 ?? skill?.description ?? "").trim();
    return normalizedDescription || "スキル詳細情報なし";
}

function getEntryNameByModalType(entry, modalType) {
    if (modalType === DETAIL_MODAL_TYPES.SKILL) {
        return getSkillName(entry);
    }
    return getItemName(entry);
}

function renderEntryBodyByModalType(entry, modalType) {
    if (modalType === DETAIL_MODAL_TYPES.SKILL) {
        return getSkillDescriptionText(entry);
    }

    const itemTypeLabel = getItemTypeLabel(entry);
    const itemDescription = getItemDescriptionText(entry);
    return `種類: ${itemTypeLabel}\n\n${itemDescription}`;
}

function ensureItemDetailModal() {
    let modal = document.getElementById(ITEM_DETAIL_MODAL_ID);
    if (modal) return modal;

    modal = document.createElement("div");
    modal.id = ITEM_DETAIL_MODAL_ID;
    modal.className = "item-detail-modal";
    modal.setAttribute("aria-hidden", "true");
    modal.innerHTML = `
        <div class="item-detail-modal-content" role="dialog" aria-modal="true" aria-labelledby="item-detail-modal-title">
            <div class="item-detail-modal-header">
                <h3 id="item-detail-modal-title" class="item-detail-modal-title">詳細</h3>
                <button type="button" class="item-detail-modal-close" aria-label="閉じる">×</button>
            </div>
            <div class="item-detail-modal-toolbar">
                <span id="item-detail-modal-type" class="item-detail-modal-type-badge">ITEM</span>
                <button type="button" class="item-detail-next-button">次へ</button>
            </div>
            <div id="item-detail-modal-caption" class="item-detail-modal-caption"></div>
            <div id="item-detail-modal-body" class="item-detail-modal-body"></div>
        </div>
    `;

    const closeButton = modal.querySelector(".item-detail-modal-close");
    if (closeButton) {
        closeButton.addEventListener("click", closeItemDetailModal);
    }

    const nextButton = modal.querySelector(".item-detail-next-button");
    if (nextButton) {
        nextButton.addEventListener("click", showNextItemDetail);
    }

    modal.addEventListener("click", (event) => {
        if (event.target === modal) closeItemDetailModal();
    });

    document.body.appendChild(modal);

    if (!itemDetailKeydownBound) {
        document.addEventListener("keydown", (event) => {
            if (event.key === "Escape") closeItemDetailModal();
        });
        itemDetailKeydownBound = true;
    }
...
function renderItemDetailModal() {
    ...
    const titleElement = modal.querySelector("#item-detail-modal-title");
    const typeElement = modal.querySelector("#item-detail-modal-type");
    const captionElement = modal.querySelector("#item-detail-modal-caption");
    const bodyElement = modal.querySelector("#item-detail-modal-body");
    const nextButton = modal.querySelector(".item-detail-next-button");
    const titleText = modal.dataset.modalTitle || "詳細";

    if (!bodyElement || !titleElement) return;

    titleElement.textContent = titleText;
    if (typeElement) {
        typeElement.textContent = itemDetailState.modalType === DETAIL_MODAL_TYPES.SKILL ? "SKILL" : "ITEM";
    }

    const items = itemDetailState.items;
    if (!Array.isArray(items) || items.length === 0) {
        if (captionElement) captionElement.textContent = "";
        bodyElement.textContent = "詳細情報なし";
        if (nextButton) nextButton.disabled = true;
        return;
    }

    const maxIndex = items.length - 1;
    if (itemDetailState.currentIndex < 0 || itemDetailState.currentIndex > maxIndex) {
        itemDetailState.currentIndex = 0;
    }
    const currentItem = items[itemDetailState.currentIndex];
    const currentNumber = itemDetailState.currentIndex + 1;
    const totalCount = items.length;
    const itemName = getEntryNameByModalType(currentItem, itemDetailState.modalType);

    if (captionElement) {
        captionElement.textContent = `${itemName} (${currentNumber}/${totalCount})`;
    }

    bodyElement.textContent = renderEntryBodyByModalType(currentItem, itemDetailState.modalType);

    if (nextButton) {
        nextButton.disabled = totalCount <= 1;
    }
}

function openDetailModal(entry, sourceItems = null, titleText = "詳細", modalType = DETAIL_MODAL_TYPES.ITEM) {
    const modal = ensureItemDetailModal();
    const sourceList = collectDetailItems(sourceItems);
    const itemList = sourceList.length > 0 ? sourceList : collectDetailItems([entry]);
    const targetIndex = Math.max(0, itemList.indexOf(entry));

    itemDetailState.items = itemList;
    itemDetailState.currentIndex = targetIndex;
    itemDetailState.modalType = modalType === DETAIL_MODAL_TYPES.SKILL ? DETAIL_MODAL_TYPES.SKILL : DETAIL_MODAL_TYPES.ITEM;
    modal.dataset.modalTitle = String(titleText || "詳細");
...
function openItemDetailModal(item, sourceItems = null, titleText = "詳細") {
    openDetailModal(item, sourceItems, titleText, DETAIL_MODAL_TYPES.ITEM);
}

function openSkillDetailModal(skill, sourceSkills = null, titleText = "詳細") {
    openDetailModal(skill, sourceSkills, titleText, DETAIL_MODAL_TYPES.SKILL);
}

function showNextItemDetail() {
    if (!Array.isArray(itemDetailState.items) || itemDetailState.items.length === 0) return;

    const totalCount = itemDetailState.items.length;
    itemDetailState.currentIndex = (itemDetailState.currentIndex + 1) % totalCount;
    renderItemDetailModal();
}

function closeItemDetailModal() {
    const modal = document.getElementById(ITEM_DETAIL_MODAL_ID);
    if (!modal) return;
...
async function displayEquipment(equipment, base, increase) {
    ...
    console.log(" 装備データをテーブルに挿入する関数: ", equipment, base, increase)
    ...
    for (const slot in equipment) {
        console.log(" 装備データをテーブルに挿入する関数 slot : ", slot)
        ...
        rowData.forEach((data, index) => {
            ...
            // 最後の列が「詳細」列（今回は含まれていないが参考用）
            else if (index === rowData.length - 1) {
                const detailButton = document.createElement("button");
                detailButton.className = "detail-button";
                detailButton.textContent = "詳細";
                detailButton.onclick = () => openItemDetailModal(item, detailItems, "詳細");
